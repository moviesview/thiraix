...
function localPosterCandidates(v) {
  ...
  for (const base of bases) {
    for (const ext of exts) list.push(`assets/thumbnail/${encodeURIComponent(base)}.${ext}`);
  }
  if (v.thumbnail) list.push(v.thumbnail);
  list.push(fallbackPoster(v.title));
  return [...new Set(list)];
}

function bindPosterFallback(img, v) {
  const sources = localPosterCandidates(v);
  let index = 0;
  const next = () => {
...
function videoCard(v, orderNo = null, masterTitle = "") {
  ...
  el.innerHTML = `
    <div class="poster">
      <img alt="${escapeHtml(v.title)}" loading="lazy" />
  ...
  bindPosterFallback(el.querySelector("img"), v);
...
function render() {
  ...
  els.collections.forEach(section => {
    ...
    grid.innerHTML = "";
    matches.forEach(({video, item, order}) => grid.appendChild(videoCard(video, order, item)));
