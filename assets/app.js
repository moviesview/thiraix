...
// Local programme artwork. The aliases intentionally include common/misspelled
// Drive filenames so every episode resolves to one clean title + local poster.
const PROGRAMMES = [
  {
    title: "Azhage Azhagu",
    slug: "azhage-azhagu",
    type: "Serial",
    thumbnail: "assets/thumbnails/azhage-azhagu.png",
    aliases: ["azhage azhagu", "azhagae azhagu", "alagey alagey", "alage alage", "azhage alage"]
  },
  {
    title: "Ayyanar Thunai",
    slug: "ayyanar-thunai",
    type: "Serial",
    thumbnail: "assets/thumbnails/ayyanar-thunai.png",
    aliases: ["ayyanar thunai", "ayiyyanar thunai", "aiyyanar thunai", "ayyanar thunai"]
  },
  {
    title: "Bigg Boss - Common Man",
    slug: "bigg-boss-common-man",
    type: "Show",
    thumbnail: "assets/thumbnails/bigg-boss-common-man.png",
    aliases: ["bigg boss common man", "big boss common man", "bigg boss comman man", "big boss comman man"]
  },
  {
    title: "Chinna Marumagal",
    slug: "chinna-marumagal",
    type: "Serial",
    thumbnail: "assets/thumbnails/chinna-marumagal.png",
    aliases: ["chinna marumagal", "chinnamarumagal"]
  },
  {
    title: "Onna Irukka Kathukanum",
    slug: "onna-irukka-kathukanum",
    type: "Serial",
    thumbnail: "assets/thumbnails/onna-irukka-kathukanum.png",
    aliases: [
      "onna irukka kathukanum", "onna iruka kathukanum", "onna irukka katthukanum",
      "onna irukka katthu irukanum", "onna irukka kaththu irukanum", "onna eruka kathukanum"
    ]
  },
  {
    ...
    slug: "pandian-stores",
    type: "Serial",
    thumbnail: "assets/thumbnails/pandian-stores.png",
    aliases: ["pandian stores", "pandiyan stores", "pandian store", "pandiyan store"]
  },
  {
    title: "Siragadikka Aasai",
    slug: "siragadikka-aasai",
    type: "Serial",
    thumbnail: "assets/thumbnails/siragadikka-aasai.png",
    aliases: ["siragadikka aasai", "siragaddika aasai", "siragadika aasai", "siragadikka asai"]
  }
];

function normalizeProgrammeKey(value = "") {
  return String(value)
    .toLowerCase()
    .replace(/\.(mkv|mp4|avi|mov|webm|m4v|ts)$/gi, " ")
    .replace(/(?:^|[_.\-\s])(?:s\d{1,3})?e\d{1,5}(?=$|[_.\-\s])/gi, " ")
    .replace(/\b(?:2160p|1080p|720p|480p|360p|jiohs|web|webrip|web-dl|hdrip|hdtv)\b/gi, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function findProgramme(value = "") {
  const key = normalizeProgrammeKey(value);
  if (!key) return null;
  return PROGRAMMES.find(programme =>
    programme.aliases.some(alias => {
      const a = normalizeProgrammeKey(alias);
      return key === a || key.startsWith(`${a} `) || key.includes(` ${a} `) || key.endsWith(` ${a}`);
    })
  ) || null;
}
...
function loadResponsiveAd(frame) {
  ...
  loadAdFrame(frame, isDesktop ? frame.dataset.desktopSrc : frame.dataset.mobileSrc);
}

function initHomeAds() {
  loadResponsiveAd(document.querySelector("#homeTopAd"));
  loadAdFrame(document.querySelector("#nativeAd"), document.querySelector("#nativeAd")?.dataset.src);
  if (window.matchMedia("(min-width: 1180px)").matches) {
    const tall = document.querySelector("#desktopTallAd");
    const medium = document.querySelector("#desktopMediumAd");
    loadAdFrame(tall, tall?.dataset.src);
    loadAdFrame(medium, medium?.dataset.src);
  }
}

function initPlayerAds() {
  loadResponsiveAd(document.querySelector("#playerBannerAd"));
  const square = document.querySelector("#playerSquareAd");
  loadAdFrame(square, square?.dataset.src);
}
...
const els = {
  ...
  volumeDown: $("#volumeDown"), volumeUp: $("#volumeUp"), share: $("#shareVideo"), download: $("#downloadVideo"), gestureLayer: $("#gestureLayer"),
  ...
  recommendedVideos: $("#recommendedVideos"), recommendedCount: $("#recommendedCount")
};
...
function normalizeVideo(video = {}) {
  const rawName = video.title || video.name || "";
  const parsed = parseDriveFilename(rawName);
  const programme = findProgramme(rawName) || findProgramme(parsed.title);
  return {
    ...video,
    title: programme?.title || parsed.title || video.title || video.name || "Untitled",
    episode: parsed.episode || video.episode || "",
    type: programme?.type || video.type || "Video",
    thumbnail: programme?.thumbnail || video.thumbnail || "",
...
function latestRecommendations(current, limit = 8) {
  return videos
    .filter(v => String(v.id) !== String(current?.id) && v.videoUrl)
    .sort((a, b) => {
      const dateDiff = (dateValue(b.date)?.getTime() || 0) - (dateValue(a.date)?.getTime() || 0);
...
function recommendedCard(v) {
  ...
  const play = () => {
    openPlayer(v);
    document.querySelector(".player-page")?.scrollTo({ top: 0, behavior: "smooth" });
  };
  el.addEventListener("click", play);
...
function renderRecommendations(current) {
  ...
  const list = latestRecommendations(current);
  els.recommendedVideos.innerHTML = "";
  list.forEach(v => els.recommendedVideos.appendChild(recommendedCard(v)));
  if (els.recommendedCount) els.recommendedCount.textContent = `${list.length} latest`;
...
function getShareUrl(v) {
  // Use a programme-specific static share page so WhatsApp/Facebook/X crawlers
  // see the correct Open Graph thumbnail before JavaScript runs.
  if (v?.programmeSlug) {
    const url = new URL(`share/${v.programmeSlug}.html`, window.location.href);
    if (v.id) url.searchParams.set("video", v.id);
    return url.toString();
  }
  const url = new URL(window.location.href);
...
if (els.download) els.download.addEventListener("click", openDownloadPage);
